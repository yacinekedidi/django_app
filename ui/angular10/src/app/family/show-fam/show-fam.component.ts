...
export class ShowFamComponent implements OnInit {
  ...
  // method that gets triggered when user clicks on the add family button
  addClick() {
    this.fam = {
      id: 0,
      cin: '',
  ...
  // method that gets triggered when user clicks on the edit button
  editClick(item) {
    this.fam = item;
  ...
  // method that gets triggered when user clicks on the more info button
  infoClick(item) {
    this.fam = item;
    this.ModalTitle = 'Family Information';
    this.ActivateInfoFamComp = true;
  }
  // method that gets triggered when user clicks on the delete family button
  deleteClick(item) {
  ...
  //triggered when the user exists a modal
  closeClick() {
    this.ActivateAddEditFamComp = false;
    this.refreshFamList();
  }

  // refreshes the familylist and the family listfilter data
  refreshFamList() {
  ...
  // filters when you type in an input box assigned for a particular row
  FilterFunction() {
    ...
    this.FamilyList = this.FamilyListWithoutFilter.filter(function (el) {
      ...
      return (
        ...
        el.cin
          .toString()
          .toLowerCase()
          .startsWith(CinFilter.toString().trim().toLowerCase()) &&
        ...
        el.address
          .toString()
          .toLowerCase()
          .includes(AddressFilter.toString().trim().toLowerCase()) &&
        el.phone
          .toString()
          .toLowerCase()
          .startsWith(PhoneFilter.toString().trim().toLowerCase()) &&
        el.health_insurance
          .toString()
          .toLowerCase()
          .includes(HealthInsuranceFilter.toString().trim().toLowerCase()) &&
        el.job
          .toString()
          .toLowerCase()
          .startsWith(JobFilter.toString().trim().toLowerCase()) &&
        el.income
          .toString()
          .toLowerCase()
          .startsWith(IncomeFilter.toString().trim().toLowerCase()) &&
        el.home_status
          .toString()
          .toLowerCase()
          .startsWith(HomeStatusFilter.toString().trim().toLowerCase()) &&
        el.home_owner
          .toString()
          .toLowerCase()
          .startsWith(HomeOwnerFilter.toString().trim().toLowerCase()) &&
        el.health_status
          .toString()
          .toLowerCase()
          .startsWith(HealthStatusFilter.toString().trim().toLowerCase()) &&
        el.deceased_parent_name
          .toString()
          .toLowerCase()
          .startsWith(
        ...
        el.cause_of_death
          .toString()
          .toLowerCase()
          .includes(CauseOfDeathFilter.toString().trim().toLowerCase()) &&
        el.sponsor_name
          .toString()
          .toLowerCase()
          .startsWith(SponsorNameFilter.toString().trim().toLowerCase()) &&
        el.family_status
          .toString()
          .toLowerCase()
          .startsWith(FamilyStatusFilter.toString().trim().toLowerCase())
      );
    });
  }

  //sorts the rows either ascending or desending order depening on the button clicked
  ...
  // downloads an excel file of the table
  download() {
    this.exportAsService
      .save(this.exportAsConfig, 'families_list')
      .subscribe(() => {
        // save started
      });
